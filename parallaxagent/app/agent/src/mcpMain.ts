...
import { parallaxDeliverable } from "./parallaxWork.js";
...
export function buildRunWork(): McpRunWork {
  return async (prompt, { abortSignal }) => {
    if (abortSignal?.aborted) throw new Error("PARALLAX work was cancelled.");
    return parallaxDeliverable(prompt);
  };
}
...
export function buildMcpServer(
  ...
  server.registerTool(
    "parallax_quote",
    {
      description: "Live PARALLAX quote for every BSC wrapper of a ticker. Does not sign.",
      inputSchema: { ticker: z.string(), usdt: z.string().optional() },
      annotations: READONLY_ANNOTATIONS,
    },
    async (a: { ticker: string; usdt?: string }) =>
      toolResult({ text: await parallaxDeliverable(JSON.stringify({ type: "quote", ticker: a.ticker, usdt: a.usdt || "10" })) }),
  );
  server.registerTool(
    "parallax_best",
    {
      description: "Best open PARALLAX rail. Does not sign.",
      inputSchema: { ticker: z.string(), usdt: z.string().optional() },
      annotations: READONLY_ANNOTATIONS,
    },
    async (a: { ticker: string; usdt?: string }) =>
      toolResult({ text: await parallaxDeliverable(JSON.stringify({ action: "best", ticker: a.ticker, usdt: a.usdt || "10" })) }),
  );
  server.registerTool(
    "parallax_weekend",
    {
      description: "PARALLAX weekend brief for a ticker. Does not sign.",
